...
// const var
var CERT_OID_VERSION = 1; // 证书版本
var CERT_OID_SERIAL = 2; // 证书序列号
var CERT_OID_SIGN_METHOD = 3; // 获取证书类型，返回RSA或SM2
var CERT_OID_ISSUER_C = 4; // 证书发放者国家名，多个之间&&&隔开
...
var CERT_OID_ISSUER_E = 10; // 证书发放者Emil地址
...
var CERT_OID_SUBJECT_E = 19; // 用户 EMAIL 地址
var CERT_OID_PUBKEY = 20; // 证书公钥
var CERT_OID_SUBJECT_DN = 33; // 用户 DN
var CERT_OID_ISSUER_DN = 34; // 颁发者 DN


// set auto logout parameters 设置页面跳转参数
...
// set user cert list id 设置显示证书列表框 ID
function SetUserCertList(strListID, certType) {
  console.log('HTML元素ID串:' + strListID)
...
// set custom usbkeychange callback 设置 USBKey 插拔回调函数
...
function $pushAllDropListBox(certUserListObj) {
  console.log('证书用户列表对象：' + JSON.stringify(certUserListObj))
  if ($_$hardCertListID != "") {
  ...
  if (certUserListObj.retVal.length === 0) {
    return console.log('没有检测到用户证书')
  }
  var allListArray = []
  while (true) {
...
function CreateSecXV2Object() {
  ...
  o.GetCertEntity = function(strCert, cb, ctx) {
    var ret = SecXV2.GetCertInfoByOid(strCert, "1.2.156.112562.2.1.1.1");
    return $myOKRtnFunc(ret, cb, ctx);
  };
...
function CreateSecXObject() {
  ...
  o.GetCertEntity = function(strCert, cb, ctx) {
    oCert.importCert(strCert, o.CERT_SRC_BASE64);
    var ret = oCert.getExtCertInfoByOID("1.2.156.112562.2.1.1.1");
    return $myOKRtnFunc(ret, cb, ctx);
  };
...
//export user signature cert 获取签名证书
...
// export user exchange cert 获取加密证书
...
//get user pin retry count 获取证书密码重试次数
...
//decrypt data by private key(not support by gm key) 私钥解密
...
//sign data with pkcs7 format Pkcs7 数据签名
...
//verify pkcs7 signed data 验证 Pkcs7 数据签名
...
//encrypt pkcs7 envlope  加密 Pkcs7 格式数字信封
...
//decrypt pkcs7 envlope  解密 Pkcs7 格式数字信封
...
//encrypt data by symmitric key  对称加密数据
...
//decrypt data by symmitric key 对称解密数据
...
//encrypt file by symmitric key 对称加密文件
...
//decrypt file by symmitric key 对称解密文件
...
//get device type return SOFT or HARD 获取证书类型
...
(function() {
  ...
  console.log('$_$GetPicObj——' + JSON.stringify($_$GetPicObj))

  $_$XTXAppObj = CreateXTXAppObject();
  console.log('$_$XTXAppObj——' + $_$XTXAppObj)
  if ($_$XTXAppObj != null) {
  ...
  console.log('$_$SecXV2Obj——' + $_$SecXV2Obj)
  if ($_$SecXV2Obj != null) {
  ...
  console.log('$_$SecXObj——' + $_$SecXObj)
  if ($_$SecXObj != null) {
  ...
  console.log('$_$WebSocketObj——' + JSON.stringify($_$WebSocketObj))
